...
namespace PCD8544
{
	namespace Graphics
	{
		constexpr std::size_t glyphSize = 8;
		constexpr std::size_t alphabetSize = std::numeric_limits<std::uint8_t>::max() + 1;

		using Glyph = std::array<std::uint8_t, glyphSize>;
		using Font = std::array<Glyph, alphabetSize>;

		static constexpr Font font =
		{ {
		...
		void DrawGlyph(const std::uint8_t line, const std::uint8_t x, const std::uint8_t & character)
		{
			...
			assert(Display::Height / 8 > line);

			const auto & glyph = font[static_cast<std::size_t>(character)];
